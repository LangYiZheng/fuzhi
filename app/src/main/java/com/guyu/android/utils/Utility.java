...
public class Utility {
	 ...
	 * @return Point
	 */
	public static Point fromWgs84ToMap(Location location,
		...
		if (4236 != spatialReference.getID())
			mapPoint = (Point) GeometryEngine.project(point, wgs84SR,
					spatialReference);
		else
			mapPoint = point;
		GpsProjectFix gpsProjectFix = GisQueryApplication.getApp().getProjectconfig()
				.getGpsProjectFix();
		if (gpsProjectFix.isEnabled()) {
			mapPoint.setX(mapPoint.getX() + gpsProjectFix.getOffsetx());
			mapPoint.setY(mapPoint.getY() + gpsProjectFix.getOffsety());
		}
		return mapPoint;
	}

	public static Point fromWgs84ToMap2(Point point,
			SpatialReference spatialReference) {
		if (spatialReference == null)
			return null;
		...
		Point mapPoint;
		if (4326 != spatialReference.getID()){
			Geometry gp = point;
			mapPoint = (Point) GeometryEngine.project(gp, wgs84SR,
					spatialReference);
		} else {
			mapPoint = point;
		}			
		...
		DebugUtils.d(TAG,"偏移前 X："+mapPoint.getX());
        DebugUtils.d(TAG,"偏移前 Y："+mapPoint.getY());
		mapPoint.setX(mapPoint.getX()+GisQueryApplication.MOVE_X);
		mapPoint.setY(mapPoint.getY()+GisQueryApplication.MOVE_Y);
        DebugUtils.d(TAG,"偏移后 X："+mapPoint.getX());
        DebugUtils.d(TAG,"偏移后 Y："+mapPoint.getY());
		return mapPoint;
	}
	 ...
	 * @return Point
	 */
	public static Point fromWgs84ToMap(Point point,
			SpatialReference spatialReference) {
		if (wgs84SR == null)
			wgs84SR = SpatialReference.create(4326);
	...
	public static void getResultsString(ToggleButton button, int count) {
		...
		if (count == 0) {
			...
			sb.append(count);
			sb.append(" ");
			sb.append((count == 1) ? resources.getString(R.string.result)
	 ...
	 *            FeatureSet
	 */
	public static void addGraphicsToFeatureLayer(
			ArcGISFeatureLayer featureLayer, FeatureSet featureSet) {
	...
	public static void addGraphicsToFeatureLayer2(
			ArcGISFeatureLayer featureLayer, FeatureSet featureSet, MapView map) {
		if (featureSet == null || featureSet.getGraphics() == null)
			return;
		...
		// Graphic gra=graphics[0];
		// poi.setRenderer(new SimpleRenderer(new SimpleFillSymbol(12)));
		// poi.addGraphics(graphics);
		// map.addLayer(poi);
		// try {
		// Log.d(TAG,"调试--"+Graphic.toJson(gra));
		// } catch (Exception e) {
		// // TODO Auto-generated catch block
		// e.printStackTrace();
		// }
		Log.d(TAG, "调试--" + map.getCenter().getX() + "-------"
				+ map.getCenter().getY());
		// map.centerAt(((Polygon)gra.getGeometry()).getPoint(0), true);
		featureLayer.setRenderer(new SimpleRenderer(new SimpleFillSymbol(12)));
		featureLayer.addGraphics(graphics);
	}
	 ...
	 * @return Point
	 */
	public static Point offsetPointInKilometers(Location location,
			double distance, double bearing, SpatialReference spatialReference) {
		...
		double lat1 = Math.asin(Math.sin(rlat) * Math.cos(rdistance)
				+ Math.cos(rlat) * Math.sin(rdistance) * Math.cos(rbearing));
		double lon1;
		if (Math.cos(lat1) == 0 || Math.abs(Math.cos(lat1)) < epsilon)
			...
			lon1 = ((rlon
					- Math.asin(Math.sin(rbearing) * Math.sin(rdistance)
							/ Math.cos(lat1)) + Math.PI) % (2 * Math.PI))
					- Math.PI;

		Point point = new Point(((lon1 * 180) / Math.PI),
				((lat1 * 180) / Math.PI));
