...
public class LeftPanel extends LinearLayout {
	...
	public LeftPanel(Context context, AttributeSet attrs) {
		super(context, attrs);
		TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.LeftPanel);
		mDuration = a.getInteger(R.styleable.LeftPanel_animationDuration, 750);		// duration defaults to 750 ms
		mPosition = a.getInteger(R.styleable.LeftPanel_position, BOTTOM);			// position defaults to BOTTOM
		mLinearFlying = a.getBoolean(R.styleable.LeftPanel_linearFlying, false);	// linearFlying defaults to false
		mOpenedHandle = a.getDrawable(R.styleable.LeftPanel_openedHandle);
		mClosedHandle = a.getDrawable(R.styleable.LeftPanel_closedHandle);
		a.recycle();
...
//			Log.d(TAG, "state: " + mState + " x: " + event.getX() + " y: " + event.getY());
	...
	private void postProcess() {
		...
		if (panelListener != null) {
			if (mIsShrinking) {
				panelListener.onPanelClosed(LeftPanel.this);
			} else {
				panelListener.onPanelOpened(LeftPanel.this);
			}
		}
	}
