...
public class VideoActivity extends Activity implements AnyChatBaseEvent {
	...
	protected SurfaceView createView() {
    	SurfaceView sfview = new SurfaceView(this);
    	sfview.setId(num++);
    	sfview.setLayoutParams(new LayoutParams(300,200));
        return sfview;
    }
	private void InitLayout() {
		...
		gridlayout = (GridLayout) findViewById(R.id.frame_remote_area);
		...
		if (AnyChatCoreSDK
			...
			for (int i = 0; i < userID.length; ++i) {
				SurfaceView  sf=createView();
				gridlayout.addView(sf, i);
				int index = anychatSDK.mVideoHelper.bindVideo(sf
						.getHolder());
				anychatSDK.mVideoHelper.SetVideoUser(index, userID[i]);
				anychatSDK.UserCameraControl(userID[i], 1);
				anychatSDK.UserSpeakControl(userID[i], 1);
			}
	...
	Runnable runnable = new Runnable() {
		...
		public void run() {
			userID = anychatSDK.GetOnlineUser();
			for (int i = 0; i < userID.length; ++i) {
				String UserName=anychatSDK.GetUserName(userID[i]);
			try {
				int videoBitrate = anychatSDK.QueryUserStateInt(userID[i],
						AnyChatDefine.BRAC_USERSTATE_VIDEOBITRATE);
				int audioBitrate = anychatSDK.QueryUserStateInt(userID[i],
				...
				{
					if (videoBitrate <= 0){						
						Toast.makeText(VideoActivity.this, UserName+"视频中断了!", Toast.LENGTH_SHORT).show();
						// 重置下，如果对方退出了，有进去了的情况
				...
				if (mFirstGetAudioBitrate){
					if (audioBitrate <= 0){
						Toast.makeText(VideoActivity.this, UserName+"音频中断了!", Toast.LENGTH_SHORT).show();
						// 重置下，如果对方退出了，有进去了的情况
	...
	protected void onPause() {
		...
		userID = anychatSDK.GetOnlineUser();
		for (int i = 0; i < userID.length; ++i) {
		anychatSDK.UserCameraControl(userID[i], 0);
		anychatSDK.UserSpeakControl(userID[i], 0);
		}
		anychatSDK.UserCameraControl(-1, 0);
		anychatSDK.UserSpeakControl(-1, 0);
	}
	...
	public void OnAnyChatUserAtRoomMessage(int dwUserId, boolean bEnter) {
		...
		if (!bEnter) {
				Toast.makeText(VideoActivity.this, UserName+"已离开！", Toast.LENGTH_SHORT).show();
			...
			gridlayout.removeAllViews();
			Toast.makeText(VideoActivity.this, UserName+"已进入！", Toast.LENGTH_SHORT).show();
			for (int i = 0; i < userID.length; ++i) {
				SurfaceView  sf=createView();
				gridlayout.addView(sf, i);
				int index = anychatSDK.mVideoHelper.bindVideo(sf
