...
public class HallActivity extends Activity implements OnItemClickListener,
	...
	private void Login() {
		String ip= GisQueryApplication.getApp().getProjectconfig().getAnyChatServerConfig().getIp();
		int  port=Integer.parseInt(GisQueryApplication.getApp().getProjectconfig().getAnyChatServerConfig().getPort());
		String strUserName=String.valueOf(SysConfig.mHumanInfo.getHumanId())+"("+SysConfig.mHumanInfo.getHumanName()+")";
		this.anychat.Connect(ip, port);
